      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-xs text-text-secondary">
            {survey.respondentCount} responses · Updated {survey.lastActivityAt ? formatRelativeDate(survey.lastActivityAt) : "recently"}
          </p>
        </div>
        <Badge variant={getStatusVariant(survey.displayStatus)}>{statusLabel}</Badge>
        <MoreHorizontal className="h-4 w-4 text-text-hint" />
      </button>
    </article>
  );
}
  ...
  return (
    <AppShell>
      {loading ? (
        ...
        <div className="survica-page-shell py-6 lg:py-8">
          <div className="space-y-5">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h1 className="text-2xl font-bold tracking-[-0.035em] text-text-primary sm:text-3xl">
                  {greeting === "Good morning" ? "Welcome back" : greeting}, {firstName} 👋
                </h1>
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <MetricCard
                label="Total surveys"
                value={surveyRows.length.toString()}
                sub="All surveys created"
                trend="20%"
              />
              <MetricCard
              ...
              <MetricCard
                ...
                sub="Across submitted responses"
                trend="12%"
              />
              <MetricCard
                label="Insights generated"
                value={metrics.pendingTranscripts.toString()}
                sub="AI-ready research findings"
                trend="30%"
              />
            </div>
              ...
              <Card hoverable={false} className="p-5">
                ...
                <div className="mt-6 h-[220px] w-full">
                  <svg viewBox="0 0 640 220" className="h-full w-full" role="img" aria-label="Response activity trend">
                    ...
                    <path d="M30 185 C90 155, 110 115, 165 140 S250 80, 310 112 S390 58, 450 88 S530 45, 620 62" fill="none" stroke="#6366F1" strokeWidth="3" strokeLinecap="round" />
                    <path d="M30 185 C90 155, 110 115, 165 140 S250 80, 310 112 S390 58, 450 88 S530 45, 620 62 L620 210 L30 210 Z" fill="url(#responseFill)" />
                    <defs><linearGradient id="responseFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stopColor="#6366F1" stopOpacity=".18" /><stop offset="1" stopColor="#6366F1" stopOpacity="0" /></linearGradient></defs>
                  </svg>
                </div>
              </Card>
              <Card hoverable={false} className="p-5">
                ...
                <div className="mt-4 divide-y divide-border">
                  {[
                    ...
                    ["View responses", () => navigate("/dashboard/surveys/q4-customer-satisfaction/results")],
                    ["Open analysis", () => navigate("/dashboard/analytics")],
                  ].map(([label, action]) => (
                    <button key={label as string} type="button" onClick={action as () => void} className="flex w-full items-center justify-between py-3 text-left text-sm font-medium text-text-primary hover:text-brand-blue"><span>{label as string}</span><ArrowUpRight className="h-4 w-4 text-text-hint" /></button>
                  ))}
                </div>
              </Card>
            </div>

            <section id="surveys" className="space-y-4 scroll-mt-8">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h2 className="text-lg font-semibold tracking-[-0.02em] text-text-primary">Recent surveys</h2>
                  <p className="mt-1 text-sm text-text-secondary">Track activity and open your latest research.</p>
                </div>
                <div className="flex flex-wrap gap-2">
                  {filters.map((item) => (
                    <Chip
